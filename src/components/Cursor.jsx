...
export default function Cursor() {
  ...
  useEffect(() => {
    // 📱 touch devices pe kuch bhi mat chalao
    ...
    const spawnPuff = (x, y) => {
      ...
      // puff banta hai → phailta hai → upar udta hai → fade hota hai → delete
      gsap.fromTo(
        puff,
        { xPercent: -50, yPercent: -50, scale: 0.4, opacity: 0.5 },
        {
          x: gsap.utils.random(-35, 35),       // halka side-drift
          y: gsap.utils.random(-60, -15),      // smoke ki tarah upar
          scale: gsap.utils.random(1.8, 2.6),  // phailna
          opacity: 0,
          duration: gsap.utils.random(0.8, 1.5),
          ease: 'power2.out',
          onComplete: () => puff.remove(),     // DOM clean rehta hai
        }
      )
    }
    ...
    const move = (e) => {
      ...
      // har ~12px ke gap pe ek puff → smooth trail, performance bhi theek
